...
export const calculateDailyMetrics = (data: TweetData[]): DailyMetrics => {
  ...
  const maxDailyEntry = Object.entries(dailyData).reduce(
  ...
  const growthRate =
    dailyCounts.length > 1
      ? ((dailyCounts[0] - dailyCounts[dailyCounts.length - 1]) /
          dailyCounts[dailyCounts.length - 1]) *
        100
      : 0;
...
export const calculateTimeOfDayStats = (data: TweetData[]): TimeOfDayStats => {
  ...
  const stats = {
    morningAvg:
      totals.morningCount > 0 ? totals.morningSum / totals.morningCount : 0,
    afternoonAvg:
      totals.afternoonCount > 0
        ? totals.afternoonSum / totals.afternoonCount
        : 0,
    eveningAvg:
      totals.eveningCount > 0 ? totals.eveningSum / totals.eveningCount : 0,
