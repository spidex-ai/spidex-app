...
export const openStandardLPPosition = async ({
  ...
  try {
    ...
    const { minAnotherAmount, maxAnotherAmount, anotherAmount } =
      transactionClient.liquidity.computePairAmount({
    ...
    const { execute, transaction } =
      await transactionClient.liquidity.addLiquidity({
        poolInfo: poolInfo as ApiV3PoolInfoStandardItem,
        ...
        amountInB: new TokenAmount(
          ...
          new Decimal(anotherAmount.toExact())
