...
export const getLpPortfolio = async (address: string): Promise<LpPortfolio> => {
  ...
  const tokens = new Set(
    lpTokens
      .map(lpToken => [lpToken.pool.mintA.address, lpToken.pool.mintB.address])
      .flat()
  );

  const prices = (
    await Promise.all(
      chunkArray(Array.from(tokens), 10).map(
        async tokenAddresses => await getPrices(tokenAddresses)
      )
    )
  ...
  const getLpTokenValue = (lpToken: LpToken) => {
    ...
    return (
      (lpToken.amount / 10 ** lpToken.decimals / pool.lpAmount) *
      (pool.mintAmountB * (prices[pool.mintB.address]!.value || 0) +
        pool.mintAmountA * (prices[pool.mintA.address]!.value || 0))
    );
  };
  ...
  return {
    ...
    totalUsd: lpTokens.reduce(
      (acc, lpToken) => acc + getLpTokenValue(lpToken),
      0
    ),
  };
};
