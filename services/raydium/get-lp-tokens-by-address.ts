...
export const getLpTokensByAddress = async (
  address: string
): Promise<LpToken[]> => {
  ...
  const lpTokens = tokenAccounts
    ...
    .filter(
      token =>
        token.mintData.mintAuthority?.toBase58() === raydiumAuthorityAddress
    );
