...
export const getPortfolio = async (wallet: string): Promise<Portfolio> => {
  const response = await queryBirdeye<PortfolioResponse>(
  ...
  const prices = (
    await Promise.all(
      chunkArray(
        response.items.map(item => parseAddress(item.address)),
        100
      ).map(async chunk => {
      ...
      if (!token) {
        try {
          ...
          return {
            ...
            valueUsd:
              item.uiAmount * (prices[parseAddress(item.address)]?.value ?? 0),
            name: metadata.name,
          ...
          console.error(
            `Failed to fetch metadata for token ${item.address}:`,
            error
          );
        }
      }
