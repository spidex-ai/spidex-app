...
export const getPostsByUserId = async (userId: string): Promise<Tweet[]> => {
  ...
  return posts.data.data.map(post => ({
    ...
    media: posts.includes.media.filter(media =>
...
export const getMentionsByUserId = async (userId: string): Promise<Tweet[]> => {
  ...
  return mentions.data.data.map(mention => ({
    ...
    media: mentions.includes.media.filter(media =>
      mention.attachments?.media_keys?.includes(media.media_key)
    ),
  }));
};

export const getMentionsByUsername = async (
  username: string
): Promise<Tweet[]> => {
