...
import {
  Transfer,
  TransferHistogramDataPoint,
  TransferParams,
  TransferResponse,
  TransferHistogramParams,
} from './types/transfer';
...
export const getTransferHistogram = (
  params: TransferHistogramParams
): Promise<TransferHistogramDataPoint[]> => {
  return request<TransferHistogramDataPoint[]>('/transfers/histogram', params);
};

export const getTransfersByTx = (
  hash: string,
  chain: string,
  transferType: 'token' | 'internal' | 'external'
): Promise<Transfer[]> => {
...
export const getAddressIntelligence = (
  address: string,
  chain?: string
): Promise<ArkhamAddress> => {
...
export const getAddressIntelligenceAll = (
  address: string
): Promise<Record<string, ArkhamAddress>> => {
  return request<Record<string, ArkhamAddress>>(
    `/intelligence/address/${address}/all`
  );
};

export const getEntityIntelligence = (
  entityId: string
): Promise<ArkhamEntity> => {
...
export const getTokenHolders = (
  ...
  const endpoint = address
    ? `/token/holders/${chainOrId}/${address}`
    : `/token/holders/${chainOrId}`;
  return request<TokenHolderResponse>(endpoint);
};

export const getTokenTopFlowByChainAndAddress = (
  chain: string,
  address: string,
  timeLast: '1h' | '24h' | '7d' | '30d'
): Promise<TokenFlow[]> => {
  return request<TokenFlow[]>(`/token/top_flow/${chain}/${address}`, {
    timeLast,
  });
};
...
export const getTokenVolume = (
  ...
  return request<TokenVolume[]>(`/token/volume/${tokenId}`, {
    timeLast,
    granularity,
  });
};
...
export const getPortfolio = (
  ...
  return request<PortfolioResponse>(`/portfolio/entity/${entityId}`, {
    time,
    chains,
  });
};
...
export const getPortfolioTimeSeries = (
  ...
  return request<PortfolioTimeSeriesResponse>(
    `/portfolio/timeSeries/entity/${entityId}`,
    { pricingId }
  );
};
...
export const getCounterparties = (
  ...
  return request<CounterpartyResponse>(
    `/counterparties/entity/${entityId}`,
    params
  );
};
