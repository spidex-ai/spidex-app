...
export class DexHunterService {
  ...
  async searchToken(
    ...
    page: number = 0,
    limit: number = 10
  ): Promise<SearchTokenInfo[]> {
    const response = await this.client.get<SearchTokenInfo[]>('swap/tokens', {
      query,
      verified,
      page,
      limit,
    ...
    const paginatedData = response
      ...
      .filter(token => token.is_verified === verified);
    return paginatedData;
  }

  async getTokenDetail(token_id: string): Promise<TokenDetail> {
    const response = await this.client.get<TokenDetail>(
      `swap/token/${token_id}`
    );
    return response;
  }
  ...
  async poolStats(
    ...
    const response = await this.client.get<PoolStatsResponse[]>(
      `stats/pools/${tokenIn}/${tokenOut}`
    );
    return response;
  }

  async estimateSwap(
    payload: EsitmateSwapPayload
  ): Promise<EsitmateSwapResponse> {
    const response = await this.client.post<EsitmateSwapResponse>(
      'swap/estimate',
      payload
    );
    return response;
  ...
  async buildSwap(payload: SwapPayload): Promise<BuildSwapResponse> {
    const response = await this.client.post<BuildSwapResponse>(
      'swap/build',
      payload
    );
    return response;
  ...
  async submitSwap(payload: SubmitSwapPayload): Promise<SubmitSwapResponse> {
    const response = await this.client.post<SubmitSwapResponse>(
      'swap/sign',
      payload
    );
    return response;
