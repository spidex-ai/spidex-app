    ...
    if (chartRef.current && ohlcvData.length > 0) {
      ...
      if (ctx) {
        ...
        for (let i = 0; i <= numYLabels; i++) {
          const y =
            height - padding - (i * (height - 2 * padding)) / numYLabels;
        ...
        for (let i = 0; i < numXLabels; i++) {
          ...
          const x =
            padding +
            (i * step * (width - 2 * padding)) / (ohlcvData.length - 1);
        ...
        ohlcvData.forEach((dataPoint, i) => {
          const x =
            padding + (i * (width - 2 * padding)) / (ohlcvData.length - 1);
          const y =
            height -
            padding -
            ((dataPoint.close - minPrice) / priceRange) *
              (height - 2 * padding);
  ...
  return (
    <div className="bg-gray-100 rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <h3 className="text-lg font-semibold mb-3 text-gray-800">
            Top Tokens by Volume ({timeframe})
          </h3>
                  ...
                  {topTokens.map((token, index) => (
                    ...
                    >
                      <td className="py-2 px-3 text-gray-800">
                        {token.ticker || token.unit || 'Unknown'}
                      </td>
                      <td className="py-2 px-3 text-right text-gray-800">
                        {token.price || 'N/A'} ADA
                      </td>
                      <td className="py-2 px-3 text-right text-gray-800">
                        {token.volume?.toLocaleString() || 'N/A'} ADA
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {loading && ohlcvData.length === 0 ? (
          ...
          <div className="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <div className="flex justify-between mt-2 text-sm text-gray-600">
              <span>
                First:{' '}
                {new Date(ohlcvData[0]?.time * 1000).toLocaleDateString()}
              </span>
              <span>
                Last:{' '}
                {new Date(
                  ohlcvData[ohlcvData.length - 1]?.time * 1000
                ).toLocaleDateString()}
              </span>
            </div>
          </div>
