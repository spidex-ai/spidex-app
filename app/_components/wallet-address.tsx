...
const WalletAddress: React.FC<Props> = ({ address, className }) => {
  const [arkhamAddress, setArkhamAddress] = useState<ArkhamAddress | null>(
    null
  );
          ...
          {arkhamAddress?.arkhamEntity && arkhamAddress?.arkhamLabel?.name ? (
            <div className="flex flex-row items-center gap-2">
              {arkhamAddress.arkhamEntity.id &&
                arkhamEntityLogos[
                  arkhamAddress.arkhamEntity
                    .id as keyof typeof arkhamEntityLogos
                ] && (
                  <Image
                    src={
                      arkhamEntityLogos[
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-row justify-between items-center">
        <h3 className="text-sm font-bold">Balances</h3>
        {portfolioLoading ? (
