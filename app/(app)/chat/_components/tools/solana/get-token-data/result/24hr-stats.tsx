...
const TwentyFourHrStats: React.FC<Props> = ({ token }) => {
  return (
    <Card className="p-2 flex flex-col gap-2">
      <h2 className="text-lg font-semibold">Volume (24hr)</h2>
      <BuySell
      ...
      <div className="flex flex-col">
        <h3 className="text-sm font-semibold">Unique Traders</h3>
        <p>
          {token.tokenAddress === 'lovelace'
            ? 'N/A'
            : `${(token['24h'].buyers + token['24h'].sellers).toLocaleString()} Traders`}
        </p>
      </div>
    </Card>
  );
