  ...
  return (
    <Card className="flex flex-col gap-2 w-full p-2">
      {/* <Table className="text-center">
        ...
        <TableBody>
          {tokens
            .slice(0, showAll ? tokens.length : 5)
            .map(([address, trade]) => (
              <TableRow key={address}>
                  ...
                  <div className="flex flex-col w-full gap-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-red-500">
                        {(
                          (trade.volume.sell /
                            (trade.volume.buy + trade.volume.sell)) *
                          100
                        ).toFixed(2)}
                        %
                      </span>
                ...
                <TableCell
                  className={
                    trade.balanceChange >= 0 ? 'text-green-500' : 'text-red-500'
                  }
                >
                ...
                <TableCell
                  className={
                    trade.usdChange >= 0 ? 'text-green-500' : 'text-red-500'
                  }
                >
                  {trade.usdChange > 0 ? '+' : '-'}$
