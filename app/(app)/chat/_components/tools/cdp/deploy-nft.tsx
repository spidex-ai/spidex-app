...
import type {
  DeployNftArgumentsType,
  DeployNftResultBodyType,
  DeployNftActionResultType,
} from '@/ai';
...
const DeployNFT: React.FC<Props> = ({ tool, prevToolAgent }) => {
  return (
    <ToolCard
      ...
      result={{
        heading: (result: DeployNftActionResultType) =>
          result.body ? `NFT Deployed` : `NFT Deployment Failed`,
        body: (result: DeployNftActionResultType) =>
          result.body ? (
            <NFTDeploymentSuccess args={tool.args} body={result.body} />
          ) : (
