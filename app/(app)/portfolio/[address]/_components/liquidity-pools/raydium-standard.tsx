      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Image
            ...
            className="h-4 w-4"
          />
          <h2 className="text-lg font-bold">Raydium Liquidity Pools</h2>
        </div>
        <p>
          $
            ...
            {portfolio.items
              .filter(
                token =>
                  ...
                  token.pool.mintA.logoURI &&
                  token.pool.mintB.logoURI &&
                  token.pool.mintA.symbol &&
                  token.pool.mintB.symbol &&
                  token.valueUsd
              )
              .map(token => (
                <TableRow key={token.pool.id}>
                  <TableCell className="font-medium flex items-center gap-2">
                    ...
                    <p>
                      {token.pool.mintA.symbol}/{token.pool.mintB.symbol} (
                      {token.pool.type})
                    </p>
                  </TableCell>
                  <TableCell>
                    {new Decimal(token.amount)
                  ...
                  <TableCell>{token.pool.week.apr}%</TableCell>
