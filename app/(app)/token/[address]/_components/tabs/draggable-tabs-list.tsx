...
>(({ className, children, selectedTab, ...props }, ref) => {
  ...
  const updateConstraints = useCallback(() => {
    if (containerRef.current && tabsListRef.current) {
      ...
      const tabsWidth = Array.from(tabsList.children).reduce(
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    if (containerRef.current) {
      ...
      const newX = Math.min(
        0,
        Math.max(constraints.right, currentX - e.deltaX - e.deltaY)
      );
  ...
  const handleDrag = (_: any, info: PanInfo) => {
    ...
    const newX = Math.min(
      0,
      Math.max(constraints.right, currentX + info.delta.x)
    );
    x.set(newX);
  };
