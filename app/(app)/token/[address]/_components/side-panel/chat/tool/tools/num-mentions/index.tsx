...
const NumMentions: React.FC<Props> = ({ tool }) => {
  return (
    <ToolCard
      ...
      result={{
        heading: (result: TokenPageNumMentionsResultType) =>
          result.body ? `Analyzed Mentions` : `Failed to Analyze Mentions`,
        body: (result: TokenPageNumMentionsResultType) =>
          result.body ? (
            <div className="flex flex-col gap-2">
              <DailyStats trendMetrics={result.body.trendMetrics} />
              <HourlyActivity
                hourlyStats={result.body.trendMetrics.hourlyStats}
              />
              <TimeOfDayPatterns
                timeOfDayStats={result.body.trendMetrics.timeOfDayStats}
              />
            </div>
          ) : (
            'No mentions found'
