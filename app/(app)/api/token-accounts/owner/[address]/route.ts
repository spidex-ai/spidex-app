...
export const GET = async (
  ...
  try {
    ...
    const tokenAccountsWithData = tokenAccounts
    ...
    const sortedTokenAccountsWithData = tokenAccountsWithData
      .sort(
        (a, b) =>
          b.amount / 10 ** a.token_data!.decimals -
          a.amount / 10 ** b.token_data!.decimals
      )
      .filter(tokenAccount => {
    ...
    const prices = priceChunks.reduce(
      (acc, chunk) => ({ ...acc, ...chunk }),
      {}
    );

    const response = sortedTokenAccountsWithData
      .map(tokenAccount => {
        return {
          ...tokenAccount,
      ...
      .sort(
        (a, b) =>
          (b.amount / 10 ** a.token_data!.decimals) * b.price -
          (a.amount / 10 ** a.token_data!.decimals) * a.price
      );
